          ...
          (response) async {
            if (response != null) {
              if (productsListType == ProductsListType.normal) {
                ...
                con.totalProducts.value = model.data?.totalCount ?? 0;
                if (model.data != null) {
                  if (isPullToRefresh) {
          ...
          (response) async {
            if (response != null) {
              ...
              if (model.data != null) {
                ...
                con.isMultipleDiamondSelection.value = con.productDetailModel.value.isDiamondMultiple ?? false;
                con.sizeId.value = con.productDetailModel.value.sizeId ?? "";
                con.productCategory.value = con.productDetailModel.value.subCategoryId ?? "";
                con.metalId.value = con.productDetailModel.value.metalId ?? "";
                con.diamondClarity.value = con.productDetailModel.value.isDiamondMultiple == true
                    ? ""
                    : (con.productDetailModel.value.diamonds != null && con.productDetailModel.value.diamonds!.isNotEmpty)
                        ? con.productDetailModel.value.diamonds?.first.diamondClarity?.value ?? ""
                        : "";
                con.diamondList.value = con.productDetailModel.value.isDiamondMultiple == true ? con.productDetailModel.value.diamonds! : [];
              }
              loader?.value = false;
            } else {
              loader?.value = false;
